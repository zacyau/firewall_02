...
class JuniperFirewallAdapter(FirewallAdapter):
    """瞻博防火墙适配器"""
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            print(f"连接瞻博防火墙 {self.name} 失败: {e}")
            return False
    ...
    def check_heartbeat(self) -> Dict[str, Any]:
        ...
        try:
            stdin, stdout, stderr = self.client.exec_command("show version")
            output = stdout.read().decode()

            stdin, stdout, stderr = self.client.exec_command("show chassis hardware")
            hardware_info = stdout.read().decode()
            ...
            return {
                ...
                "hardware_info": hardware_info[:200]
            }
        except Exception as e:
            self.disconnect()
            return {
    ...
    def generate_cli_command(self, policy_config: Dict[str, Any]) -> str:
        """生成瞻博防火墙CLI命令"""
        return self._render_template("juniper_policy.j2", policy_config)
